...
export function getConfiguredManagedAgentResources(): ManagedAgentResources | null {
  return {
    agentId: process.env.OPENAI_MANAGED_AGENT_ID || 'openai-ooh-agent',
    environmentId: process.env.OPENAI_MANAGED_ENVIRONMENT_ID || 'openai-responses',
  }
}

export async function createManagedAgentResources(): Promise<ManagedAgentResources> {
  return getConfiguredManagedAgentResources() as ManagedAgentResources
}

export async function startAgencySession(input: {
  area: LatLng
  brief: string
...
export function buildCampaignRunPrompt(area: LatLng, brief: string): string {
  return [
    'Run as an AI-operated physical marketing / out-of-home agency desk.',
    '',
    ...
    '4. Simulate pre-flight scoring conceptually: visibility, dwell time, clutter, viewing angle, audience fit, creative readability, and inquiry priority.',
...
function buildAgentSystemPrompt(): string {
  return [
    'You are an AI-operated OOH agency desk for physical advertising campaigns.',
...
function buildPedestrianInterviewSystemPrompt(): string {
  return [
    'You are a field researcher for an OOH advertising analytics platform.',
